...
export async function getTotalInPutOctetsPerPerson() {
  const result = await liaisonDB.query({
    query: `SELECT acctuniqueid, SUM(acctinputoctets) AS total_data_received
            FROM hm_stats.sessions
...
export async function getAvgInPutOctetsPerPerson() {
  const result = await liaisonDB.query({
    query: `SELECT acctuniqueid, AVG(acctinputoctets) AS average_data_received
            FROM hm_stats.sessions
            GROUP BY acctuniqueid
            LIMIT 5`,
    format: 'JSON',
...
export async function getMinInPutOctetsPerPerson() {
  const result = await liaisonDB.query({
    query: `SELECT acctuniqueid, MIN(acctinputoctets) AS min_data_received
            FROM hm_stats.sessions
            GROUP BY acctuniqueid
            LIMIT 5`,
    format: 'JSON',
...
export async function getMaxInPutOctetsPerPerson() {
  const result = await liaisonDB.query({
    query: `SELECT acctuniqueid, MAX(acctinputoctets) AS max_data_received
            FROM hm_stats.sessions
            GROUP BY acctuniqueid
            LIMIT 5`,
    format: 'JSON',
